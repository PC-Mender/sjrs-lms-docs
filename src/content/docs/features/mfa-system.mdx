...
- `GET /api/v1/auth/mfa/status` - Get MFA status (includes recent activity logs)
- `POST /api/v1/auth/mfa/disable` - Disable MFA (requires password & email OTP)
- `POST /api/v1/auth/mfa/challenge/verify` - Verify MFA code during login (supports TOTP & Backup Codes)
- `POST /api/v1/auth/mfa/backup-codes/regenerate` - Generate new backup codes (requires password)

### Security Considerations

1. **Enforcement**
   - The login endpoint (`/api/v1/auth/login`) checks `user_mfa_settings`.
