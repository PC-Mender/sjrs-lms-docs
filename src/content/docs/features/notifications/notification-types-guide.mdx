...
The app exposes these notification areas under the header **Notifications** menu. All roles use the unified `/dashboard/` route.
...
- **System Notifications**: `/dashboard/system-notifications`
  - Requires `notification_events` permissions.
- **App Notifications Log**: `/dashboard/app-notifications-log`
...
### Frontend Components
- **Admin Page**: `/dashboard/system-notifications` (requires `notification_events` resource permissions)
- **Component**: `src/pages/dashboard/notification-center.tsx`
...
### Notification Feed
The user notification feed (`/api/v1/notifications/feed`) combines:
