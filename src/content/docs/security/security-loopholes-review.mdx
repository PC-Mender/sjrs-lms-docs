...
**Evidence**:
- `functions/api/auth/registration.ts` - Email is normalized before rate limiting
- `functions/api/auth/registration.ts` - Rate limit uses `normalizedEmail`
...
**Evidence**:
- `functions/api/auth/registration.ts` - UserType is canonicalized and validated
...
**Evidence**:
- `functions/middleware/security/turnstile-validation.ts` - Fail-closed in production, fail-open only in development
...
**Previous Issue**: 
- `functions/middleware/auth/password/hashing.ts` - Fallback to a weaker SHA-256-based hashing method if PBKDF2 fails
...
**Fixes Applied**:
1. Email is normalized before storage (`functions/api/auth/registration.ts`)
2. Database unique constraint error handling added (`functions/middleware/auth/users/creation.ts`)
3. Race condition handling in registration handler (`functions/api/auth/registration.ts`)
...
**Evidence**:
- `functions/api/auth/registration.ts` - Returns generic success message even if email exists
...
**Previous Issue**:
- `functions/api/auth/session-management.ts` - `console.warn` used
...
**Fixes Applied**:
1. Added `isProduction(env)` and `isDevelopment(env)` helpers in `functions/lib/env.ts`
...
**Fixes Applied**:
- Updated rate limiting to use composite key: `${normalizedEmail}:${remoteip}` (`functions/api/auth/registration.ts`)
...
**Fixes Applied**:
1. Added unique constraint error detection in `createUser()` (`functions/middleware/auth/users/creation.ts`)
2. Added race condition handling in registration handler (`functions/api/auth/registration.ts`)
...
**Evidence**:
- `functions/api/auth/registration.ts` - `validateEmailDomain()` is called
