...
### API Calls
```typescript
// CRUD hooks (recommended — auto-prepends v1/ prefix)
...
// Direct API calls (must include v1/ prefix)
import { d1Request } from '../utilities/api/d1Client';

const { data } = await d1Request('/api/v1/books', { method: 'GET' });
```
...
export const FeaturePage: React.FC = () => {
  const { data, isLoading, error } = useStandardQuery({
    queryKey: ['feature'],
    queryFn: () => d1Request('/api/v1/feature'),
    preset: 'REAL_TIME',
  });
...
// Note: All API routes use /api/v1/ prefix (normalized to /api/ by the pipeline)
import { getAuthenticatedUser } from '../../middleware/auth';
import { withSecurity } from '../../middleware/security';
import { createSuccessResponse, createPaginatedResponse } from '../../utilities/response-builder';
...
export async function feature(request: Request, env: any) {
  ...
  try {
    ...
    if (request.method === 'GET' && (path === '' || path === '/')) {
      ...
      return createPaginatedResponse(result.results, result.results.length, 1, 20);
    }
    ...
    return await handleError(error, {
      ...
      context: { endpoint: '/api/v1/feature' }
    });
  }
}
```

See [API Versioning](/architecture/api-versioning) for details on the `/api/v1/` prefix.
...
const validation = await validateInput(data, schema);
if (!validation.isValid) {
  throw new Error(validation.errors.map(e => e.message).join(', '));
}
```

See [Validation System](/development/validation-system) for full details on the Zod-based validation architecture.
...
```typescript
import { useStandardQuery, useStandardMutation } from '../../utilities/query';

// Query (uses centralized preset system)
const { data, isLoading } = useStandardQuery({
  queryKey: ['books'],
  queryFn: () => d1Request('/api/v1/books'),
  preset: 'REAL_TIME',
});

// Mutation (automatic cache invalidation via invalidateKeys)
const mutation = useStandardMutation({
  mutationFn: (variables) => d1Request('/api/v1/books', {
    method: 'POST',
    body: variables,
  }),
