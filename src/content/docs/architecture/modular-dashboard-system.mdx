...
<Aside type="caution" title="Archived">
This was a design proposal for a widget registry system (`src/registry/dashboard-widgets.ts`, `src/factories/widget-factory.ts`). The dashboard has since been consolidated to a single `/dashboard` route with role-based lazy-loaded views using `ROLE_COMPONENT_MAP`. The widget customization concepts may be revisited, but this document does not reflect the current implementation. See [Routing System](../architecture/routing-system) for current architecture.
</Aside>
