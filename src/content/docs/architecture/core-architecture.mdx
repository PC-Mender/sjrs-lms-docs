...
├── index.css                # Global styles (responsive CSS classes, design tokens)
├── components/              # Reusable UI components
│   ├── auth/               # Auth guards, login forms, MFA
│   ├── layout/             # Layout wrappers, header, footer
│   ├── features/           # Feature-specific UI components
│   └── ui/                 # Shared UI primitives
├── pages/                   # Route components (SPA routing)
│   ├── dashboard.tsx       # Unified dashboard (role dispatcher → lazy-loads role views)
│   ├── login.tsx           # Authentication
│   ├── [feature]/          # Feature-specific pages (e.g., roles/, borrow-limits/)
│   └── ...                  # Other page components
├── router/                  # Routing system
│   ├── AppRouter.tsx       # Main router with AuthGuard
│   ├── route-definitions.ts # Route groups, dynamic generation
│   ├── components/         # DashboardSubRouter, error boundaries, loaders
│   ├── config/             # Feature route definitions
│   └── utils/              # Route component loader, lazy loading
├── hooks/                   # Custom React hooks (d1 data hooks, auth, permissions)
├── contexts/                # React context providers (auth, theme)
├── utilities/               # Helper functions (query, crud, roles, etc.)
├── constants/               # Application constants, configs
├── types/                   # TypeScript definitions
└── services/                # API service layer
```

**Note**: The frontend is a **Single Page Application (SPA)**. All routing is handled client-side by React Router, meaning the server serves a single `index.html` file and React Router manages all navigation and page rendering on the client side. The canonical dashboard entry point is `/dashboard` for all roles — role-specific rendering is handled by the Dashboard page via `ROLE_COMPONENT_MAP` which lazy-loads the appropriate role view (e.g., `dashboard-admin.tsx`, `dashboard-librarian.tsx`). Legacy role-prefixed paths (`/dashboard-superuser`, `/dashboard-admin`, etc.) redirect to `/dashboard`.
...
- **JWT-based authentication** (HttpOnly cookies with `__Host-` prefix)
- **CSRF protection** via signed double-submit cookie pattern
- **Input validation** via Zod schemas on all backend endpoints
...
### URI Versioning
All API routes use the `/api/v1/` prefix. The main request pipeline (`functions/index.ts`) normalizes `/api/v1/<resource>` to `/api/<resource>` before route matching, so handlers only strip `/api/<resource>`. Legacy `/api/<resource>` paths (without `v1/`) are rejected with HTTP 400. Frontend calls use `/api/v1/` paths exclusively, and the `createCrudHooks` factory automatically prepends `v1/` to the resource parameter.
...
export async function resourceName(request: Request, env: any) {
  ...
  if (request.method === 'GET' && (path === '' || path === '/')) {
    // Handle GET /api/v1/resourceName
  } else if (request.method === 'POST' && (path === '' || path === '/')) {
    // Handle POST /api/v1/resourceName
  } else if (path.startsWith('/')) {
    const id = path.substring(1);
    // Handle /api/v1/resourceName/:id
  }
}
```
...
// Root endpoint: /api/v1/resourceName
if (path === '' || path === '/') {
  // Handle collection operations (GET, POST)
}

// Specific resource: /api/v1/resourceName/:id
if (path.startsWith('/')) {
  const id = path.substring(1);
...
All API responses must use standardized JSON envelopes from `functions/utilities/response-builder.ts` (`createSuccessResponse()`, `createPaginatedResponse()`, `createCreatedResponse()`, `createUpdatedResponse()`, `createDeletedResponse()`, `createErrorResponse()`). Never use raw `new Response(JSON.stringify({ ... }))`.

### Legacy Permissions Compatibility
Legacy permission endpoints are preserved for backward compatibility and internally routed to the unified `/api/v1/permissions` service.
...
try {
  ...
  return await handleError(error, {
    ...
    context: { endpoint: '/api/v1/resourceName' }
  });
}
```
...
```typescript
// Normalize /api/v1/<resource> → /api/<resource> before matching
// Reject legacy /api/<resource> (without v1/) with HTTP 400
...
# Quality
npm run quality-gates       # Run all quality gates (lint, test, build)

# Testing
npm run test               # Run unit tests (Vitest)
npm run test:e2e           # Run E2E tests (Playwright, against staging)
