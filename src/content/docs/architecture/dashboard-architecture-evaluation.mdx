...
This document was a pre-migration evaluation of the role-specific dashboard architecture. The dashboard has since been consolidated to a single `/dashboard` route with role-based lazy-loaded views. See [Routing System](/architecture/routing-system) for the current architecture.
</Aside>
