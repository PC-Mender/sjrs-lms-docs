...
> Important (GitHub Actions runtime deprecation)
>
> GitHub has deprecated Node.js 20 for JavaScript actions. In this workflow, `actions/checkout@v4` and `actions/setup-node@v4` are flagged and may become unreliable.
>
> Timeline:
> - Node.js 24 becomes default for actions on June 2, 2026
> - Node.js 20 is removed from runners on September 16, 2026
>
> Recommended updates:
> - Upgrade to action versions that explicitly support Node.js 24 (for example, newer major versions of `actions/checkout` and `actions/setup-node` when available in your repository policy)
> - Optionally opt in now by setting `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` in workflow `env`
> - Temporary fallback after default switch: `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION=true`
>
...
> Important (GitHub Actions runtime deprecation)
>
> The same Node.js 20 deprecation applies to this job when using `actions/checkout@v4` and `actions/setup-node@v4`.
>
> Ensure workflow action versions are updated for Node.js 24 compatibility before enforcement dates:
> - Node.js 24 default: June 2, 2026
> - Node.js 20 removal: September 16, 2026
>
> Temporary compatibility env vars:
> - Opt in now: `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`
> - Temporary opt out (after default switch): `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION=true`
...
4. Sends POST request to `/api/v1/notifications/events/ingest`
...
2. **Notification Center** (`src/pages/dashboard/notification-center.tsx`)
...
1. **Creation** (`scripts/notify-deployment.js`)
   ...
   - Sends POST to `/api/v1/notifications/events/ingest`
...
3. **Display** (`src/pages/dashboard/notification-center.tsx`)
   - Fetches from `/api/v1/notifications/events`
...
Edit `src/pages/dashboard/notification-center.tsx`:
...
- `src/pages/dashboard/notification-center.tsx` - Notification display UI
