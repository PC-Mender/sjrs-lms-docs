...
- Permissions API: `functions/api/permissions/index.ts`
  - POST `/api/v1/permissions/*` – unified permissions operations
  - Legacy routes mapped internally (see Legacy Compatibility)

## Legacy Compatibility

These legacy endpoints are supported and internally routed to `/api/v1/permissions`:

- `POST /api/v1/role_permissions`
- `POST /api/v1/permission_resources`
- `POST /api/v1/permission_actions`
- `POST /api/v1/permission_audit_log`
- `POST /api/v1/update_role_permission`
...
**The backend permissions endpoint (`/api/v1/permissions/check`) is the single source of truth for all permission decisions.**
