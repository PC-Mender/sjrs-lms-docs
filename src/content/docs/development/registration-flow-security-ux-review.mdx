...
- Hardened `/api/v1/auth/check-email` to prevent email enumeration while adding IP-based rate limiting (`functions/api/auth/check-email.ts`, `functions/middleware/kv-utils/config/kv-configs.ts`).
...
**Issue:** The `/api/v1/auth/check-email` endpoint reveals whether an email address is registered in the system, enabling attackers to enumerate valid user emails.
