...
The pilot is complete. The unified dashboard is now the production implementation — all roles use `/dashboard` with role-based lazy-loaded views. The `VITE_USE_UNIFIED_DASHBOARD` feature flag and legacy implementations have been removed. See [Routing System](../architecture/routing-system) for current details.
</Aside>
