...
- Prefer the `usePermissions` hook for bulk UI gating. It hydrates via `/api/v1/role_permissions`, falls back gracefully when the bulk path fails, and keeps caches tidy through `refreshPermissions()` and event emitters.
...
- Permission metadata (resources/actions) is available via `/api/v1/permission_resources` and `/api/v1/permission_actions`; the updated client fallbacks rely on those endpoints to prevent drift.
