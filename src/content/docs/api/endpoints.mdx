...
- **GET** `/api/books/sections` - Get distinct book sections with assigned books
- **GET** `/api/books/:id` - Get specific book
- **POST** `/api/books` - Create book (requires `books:create`)
- **PUT** `/api/books/:id` - Update book (requires `books:update`)
- **DELETE** `/api/books/:id` - Delete book (requires `books:delete`)
...
- **POST** `/api/loans` - Create loan (requires `loans:create`)
- **PUT** `/api/loans/:id` - Update loan (requires `loans:update`)
- **DELETE** `/api/loans/:id` - Delete loan (requires `loans:delete`)
- **POST** `/api/loans/:id/renew` - Request/perform loan renewal (permission-gated)
- **POST** `/api/loans/:id/renew/approve` - Approve a renewal request (requires `loans:update`)
- **POST** `/api/loans/:id/renew/reject` - Reject a renewal request (requires `loans:update`)
